...
```html
<!-- Wrapper initialisé par FileField -->
<div class="file-dropzone" data-file-dropzone data-name="form[photo]" ...>

    <!-- Input navigateur (masqué, sans name ni id) -->
    <input type="file" hidden accept="image/jpeg,image/png">

    <!-- Zone de dépôt -->
    <div class="header">
        <div class="icon"></div><!-- Icône fichier FA via ::before -->
        <div>
            <span class="placeholder">Glisser & Déposer</span>
            <span class="placeholder">ou</span>
            <span class="placeholder browse">Cliquer pour rechercher</span>
            <span class="support">Fichiers acceptés : jpeg, png</span>
        </div>
    </div>

    <!-- Zone d'erreurs (vide par défaut) -->
    <div class="errors"></div>

</div>
```

### État après sélection de fichiers
...
<div class="file-dropzone" data-file-dropzone ...>
    ...
    <ul class="list-files">
        <li class="file-item">
            <!-- Input fichier réel — soumis avec le formulaire -->
            <input type="file" name="form[photo]" hidden>
            <!-- Icône MIME -->
...
```html
<!-- La classe .dragover est ajoutée pendant le survol -->
<div class="file-dropzone dragover" ...>…</div>
```
